...
func outputAgentPrimeText(cmd *cobra.Command, output agentPrimeOutput) error {
	...
	if output.TeamContext != nil {
		...
		// emit MANDATORY read instructions for team AGENTS.md (critical for team context)
		if ci := output.TeamContext.ClaudeInstructions; ci != nil && ci.HasAgentsMD {
			fmt.Fprintln(cmd.OutOrStdout())
			fmt.Fprintln(cmd.OutOrStdout(), "## MANDATORY: Read Team AGENTS.md")
			fmt.Fprintln(cmd.OutOrStdout())
			fmt.Fprintln(cmd.OutOrStdout(), "Your team has shared guidance that MUST be loaded before any work:")
			fmt.Fprintln(cmd.OutOrStdout())
			fmt.Fprintf(cmd.OutOrStdout(), "  **READ NOW**: `%s`\n", shortenPath(output.TeamContext.ClaudeInstructions.AgentsMDPath))
			fmt.Fprintln(cmd.OutOrStdout())
			fmt.Fprintln(cmd.OutOrStdout(), "This file contains team norms, conventions, and architectural decisions that")
			fmt.Fprintln(cmd.OutOrStdout(), "influence how you should plan and implement work in this repository.")
			fmt.Fprintln(cmd.OutOrStdout())
			fmt.Fprintln(cmd.OutOrStdout(), "**You MUST use the Read tool to load this file IMMEDIATELY.**")
			fmt.Fprintln(cmd.OutOrStdout(), "Do not proceed with any planning or implementation until you have read it.")
		}

		// emit optional CLAUDE.md if it exists (separate from mandatory AGENTS.md)
		if ci := output.TeamContext.ClaudeInstructions; ci != nil && ci.HasClaudeMD {
			fmt.Fprintln(cmd.OutOrStdout())
			fmt.Fprintln(cmd.OutOrStdout(), "## Team Claude Configuration")
			fmt.Fprintln(cmd.OutOrStdout())
			fmt.Fprintf(cmd.OutOrStdout(), "Additional Claude-specific config: `%s`\n", shortenPath(output.TeamContext.ClaudeInstructions.ClaudeMDPath))
		}
